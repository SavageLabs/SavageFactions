...
public enum UpgradeType {
    ...
    public int getSlot() {
        ...
        return config.getInt("fupgrades.MainMenu." + this.id + ".DisplayItem.Slot");
    }
    ...
    public ItemStack buildAsset(Faction f) {
        ConfigurationSection config = Files.CONFIG.getFile().getConfigurationSection("fupgrades.MainMenu." + this.id + ".DisplayItem");
       // ConfigurationSection config = SavageFactions.plugin.getConfig().getConfigurationSection("fupgrades.MainMenu." + this.id + ".DisplayItem");
        ...
        meta.setLore(Methods.plList(replacePlaceholders(config.getStringList("Lore"), new Placeholder("{level}", level + ""))));
