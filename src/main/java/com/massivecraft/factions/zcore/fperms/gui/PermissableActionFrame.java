...
public class PermissableActionFrame {
    ...
    public PermissableActionFrame(Faction f) {
        section = SavageFactions.plugin.getConfig().getConfigurationSection("fperm-gui.action");
        gui = new Gui(SavageFactions.plugin,
                section.getInt("rows", 3),
                SavageFactions.plugin.color(SavageFactions.plugin.getConfig().getString("fperm-gui.action.name").replace("{faction}", f.getTag())));
    }
